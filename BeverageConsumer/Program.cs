...
string? githubToken = Environment.GetEnvironmentVariable("GHUB_PAT");
string? modelName = config["ai:model"] ?? "gpt-4o";

// MCP Client Transport using HTTP (Docker)
string? mcpDockerCmd = config["mcpServerDockerCommand"] ?? "docker run -i --rm superlordwilliam/beveragemcpserve";
